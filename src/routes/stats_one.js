...
async function getStats(request, reply) {
  ...
  try {
    // Single AQL query using subqueries (LET) so we only round-trip once.
    const query = `
      LET limitVal = @limit
      ...
      RETURN { LastVul, LastKev, LastRansonware, LastEpss, LastExploit }
    `;

    const cursor = await db.query(query, { limit: parsedLimit });
